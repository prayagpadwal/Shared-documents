...
-- Description:	
-- =============================================
CREATE PROCEDURE PDCStoredProcedure2 
	-- Add the parameters for the stored procedure here
	@WeekFilter int = 38, 
	@ActualsFilter DATE = '2024-09-28',
	@RAG_MIN_Threshold DECIMAL = 0.995,
	@RAG_MAX_Threshold DECIMAL	= 1.25,
	@Output VARCHAR = 'PDC',
	@Year INT = 2024
AS
BEGIN
	...
	--SELECT @WeekFilter, @ActualsFilter, @RAG_MIN_Threshold, @RAG_MAX_Threshold
...
            ,CASE 
                  WHEN ISNULL(dot.[Initiation Target],0) > 0 and ISNULL(act.[Initiation Actuals],0) = 0 THEN 0
                  WHEN ISNULL(dot.[Initiation Target],0) = 0 THEN 1
                  ELSE ROUND((act.[Initiation Actuals]/ISNULL(COALESCE(dot.[Initiation Target],0),0)),4)
            END AS [% to Plan]
            ,'' AS [On Track]
            ,CASE
                  WHEN ROUND(COALESCE(act.[Initiation Actuals],0)/ISNULL(COALESCE(dot.[Initiation Target],0),0),2) > @RAG_MAX_Threshold
                        THEN 'RED'
                  WHEN ROUND(COALESCE(act.[Initiation Actuals],0)/ISNULL(COALESCE(dot.[Initiation Target],0),0),2) < @RAG_MIN_Threshold
                        THEN 'RED'
                  ELSE 'GREEN'
             END AS [RAG]
...
            ,CASE 
                  WHEN ISNULL(dot.[Design Target],0) > 0 and ISNULL(act.[Design Actuals],0) = 0 THEN 0
                  WHEN ISNULL(dot.[Design Target],0) = 0 THEN 1
                  ELSE ROUND((act.[Design Actuals]/NULLIF(COALESCE(dot.[Design Target],0),0)),4)
            END AS [% to Plan]
            ,'' AS [On Track]
            ,CASE
                  WHEN dot.[PDC Program] IN ('ED WFM Non-Exempt Expulsion Fuses HFTD','ED WFM Line Sensors', 'Mobile Home Park Electric','ED WFM Surge Arresters'
                                                                  ,'COE Expense','Maintenance Expense')
                        THEN 'BLACK'
                  WHEN ROUND(COALESCE(act.[Design Actuals],0)/NULLIF(COALESCE(dot.[Design Target],0),0),2) > @RAG_MAX_Threshold
                        THEN 'RED'
                  WHEN ROUND(COALESCE(act.[Design Actuals],0)/NULLIF(COALESCE(dot.[Design Target],0),0),2) < @RAG_MIN_Threshold
                        THEN 'RED'
                  ELSE 'GREEN'
             END AS [RAG]
...
            ,CASE 
                  WHEN ISNULL(dot.[Dependency Target],0) > 0 and ISNULL(act.[Dependency Actuals],0) = 0 THEN 0
                  WHEN ISNULL(dot.[Dependency Target],0) = 0 THEN 1
                  ELSE ROUND((act.[Dependency Actuals]/NULLIF(COALESCE(dot.[Dependency Target],0),0)),4)
            END AS [% to Plan]
            ,'' AS [On Track]
            ,CASE
                  WHEN dot.[PDC Program] IN ('Mobile Home Park Electric') THEN 'BLACK'
                  WHEN ROUND(COALESCE(act.[Dependency Actuals],0)/NULLIF(COALESCE(dot.[Dependency Target],0),0),2) > @RAG_MAX_Threshold
                        THEN 'RED'
                  WHEN ROUND(COALESCE(act.[Dependency Actuals],0)/NULLIF(COALESCE(dot.[Dependency Target],0),0),2) < @RAG_MIN_Threshold
                        THEN 'RED'
                  ELSE 'GREEN'
             END AS [RAG]
...
            ,CASE 
                  WHEN ISNULL(dot.[Schedule Target],0) > 0 and ISNULL(act.[Schedule Actuals],0)= 0 THEN 0
                  WHEN ISNULL(dot.[Schedule Target],0) = 0 THEN 1
                  ELSE ROUND((act.[Schedule Actuals]/NULLIF(COALESCE(dot.[Schedule Target],0),0)),4)
            END AS [% to Plan]
            ,'' AS [On Track]
            ,CASE
                  WHEN ROUND(COALESCE(act.[Schedule Actuals],0)/NULLIF(COALESCE(dot.[Schedule Target],0),0),2) > @RAG_MAX_Threshold
                        THEN 'RED'
                  WHEN ROUND(COALESCE(act.[Schedule Actuals],0)/NULLIF(COALESCE(dot.[Schedule Target],0),0),2) < @RAG_MIN_Threshold
                        THEN 'RED'
                  ELSE 'GREEN'
             END AS [RAG]
...
            ,CASE 
                  WHEN ISNULL(dot.[Execute Target],0) > 0 and ISNULL(act.[Execute Actuals],0) = 0 THEN 0
                  WHEN ISNULL(dot.[Execute Target],0) = 0 THEN 1
                  ELSE ROUND((act.[Execute Actuals]/NULLIF(COALESCE(dot.[Execute Target],0),0)),4)
            END AS [% to Plan]
            ,'' AS [On Track]
            ,CASE
                  WHEN ROUND(COALESCE(act.[Execute Actuals],0)/NULLIF(COALESCE(dot.[Execute Target],0),0),2) > @RAG_MAX_Threshold
                        THEN 'RED'
                  WHEN ROUND(COALESCE(act.[Execute Actuals],0)/NULLIF(COALESCE(dot.[Execute Target],0),0),2) < @RAG_MIN_Threshold
                        THEN 'RED'
                  ELSE 'GREEN'
             END AS [RAG]
...
                  --when dot.pdc program  = line sensors add ed wfm dist fault antic completions
UNION ALL
